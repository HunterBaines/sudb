...
class TestGeneratorUniqueness(unittest.TestCase):
    ...
    def test_uniqueness(self):
        # NB: Testing this well requires generating from many seeds, which
        # takes a long time (on my machine, running this test with 1001
        # seeds takes about 11.3 minutes). The last change to the RNG
        # occurred on 2017 December 7 in commits a428a04a and 9d2f13e; as
        # of those commits and that date, this passes for seeds between 0
        # and 10000 inclusive
        puzzle_hash_dict = {}
        repeated_puzzles = []

        for seed in range(self.START_SEED, self.END_SEED + 1):
            # A waste of time to duplicate puzzle; a waste of space to
            # store it in dict
        ...
        # Designed to show number of unique puzzles relative to number
        # possible if assertion fails
